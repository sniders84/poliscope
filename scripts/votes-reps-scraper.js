...
async function fetchAllVotes() {
  const base = `https://api.congress.gov/v3/rollcall-vote?congress=119&chamber=house&api_key=${API_KEY}&limit=250`;
  let offset = 0;
  const votes = [];
  while (true) {
...
(async () => {
  ...
  console.log(`Saved House votes: ${votes.length} votes`);
})().catch(err => {
