 ...
 * - Covers all sessions of the given Congress
...
async function run() {
  ...
  for (const session of sessions) {
    const bills = await fetchAllPages(
      `https://api.congress.gov/v3/bill?format=json&congress=${CONGRESS}&chamber=Senate&session=${session}&api_key=${API_KEY}`
    );
    const amendments = await fetchAllPages(
      `https://api.congress.gov/v3/amendment?format=json&congress=${CONGRESS}&chamber=Senate&session=${session}&api_key=${API_KEY}`
    );
