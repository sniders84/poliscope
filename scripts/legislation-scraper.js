...
const API_KEY = process.env.CONGRESS_API_KEY;   // <-- match your secret name
...
function becameLaw(b) {
  ...
  return la.includes('became public law') || la.includes('enacted') ||
         (Array.isArray(b?.laws) && b.laws.length > 0);
}
...
async function run() {
  ...
  const bills = await fetchAllPages(
    `https://api.congress.gov/v3/bill?format=json&congress=${CONGRESS}&chamber=Senate&api_key=${API_KEY}`
  );
  const amendments = await fetchAllPages(
    `https://api.congress.gov/v3/amendment?format=json&congress=${CONGRESS}&chamber=Senate&api_key=${API_KEY}`
  );
  ...
  for (const amdt of amendments) {
    ...
    for (const c of cosponsors) {
      ...
      if (cid) ensure(cid).cosponsoredAmendment++;
    }
  }
