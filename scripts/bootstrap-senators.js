...
(async () => {
  ...
  const withId = sens.filter(s => s.congressgovId).length;
  console.log(`Found ${sens.length} current Senators in local roster`);
  console.log(`Of which ${withId} have bioguideId (set as congressgovId)`);
  if (withId > 0) {
    console.log('Sample with IDs (first 3):', 
      sens.slice(0, 3).map(s => ({ name: s.name, state: s.state, congressgovId: s.congressgovId }))
    );
  } else {
    console.warn('WARNING: No bioguideIds found in legislators-current.json — check file format!');
  }
